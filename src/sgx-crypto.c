		...
		size_t entry_offset)
{
	...
	if (!tmp_tcs)
		printk(KERN_INFO  "failed to allocate tcs");
...
void rsa_key_generate(uint8_t *pubkey, uint8_t *seckey, rsa_context *rsa, int bits)
{
	...
	ret = ctr_drbg_init(&ctr_drbg, entropy_func, &entropy,
			...
			strlen(pers));
	if (ret)
		printk(KERN_INFO  "failed to initiate entropy: %d", ret);
	...
	if (ret)
		printk(KERN_INFO  "failed to generate RSA keys: %d", ret);
...
{
	//char **fp = fopen(conf, "r");
	char **fp = NULL;
	if (!fp)
		printk(KERN_INFO  "failed to locate %s", conf);
	...
	pk =kmalloc(bytes, GFP_KERNEL);
	sk =kmalloc(bytes, GFP_KERNEL);
	p =kmalloc(bytes, GFP_KERNEL);
	q =kmalloc(bytes, GFP_KERNEL);
	e =kmalloc(bytes, GFP_KERNEL);
	...
	rsa_context *ctx = kmalloc(sizeof(rsa_context), GFP_KERNEL);
	if (!ctx)
		printk(KERN_INFO  "failed to allocate rsa ctx");
		...
		unsigned char *bytes, int len)
{
	...
	int ret = rsa_pkcs1_sign(ctx, NULL, NULL, RSA_PRIVATE,
			...
			(unsigned char *)sig);
	if (ret)
		printk(KERN_INFO  "failed to sign: 0x%x", -ret);
}
...
uint8_t *alloc_pkcs1_5_padding(void) {
	...
	uint8_t *pkcs1_5_padding = (uint8_t *)kmalloc(sizeof(uint8_t)* 352, GFP_KERNEL);
	int i;
