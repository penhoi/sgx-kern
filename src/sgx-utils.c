...
{
	char *buf = kmalloc(size*2 + 1, GFP_KERNEL);
	if (!buf)
		return NULL;
...
{
	char *msg = kmalloc(5120, GFP_KERNEL);
	if (!msg)
		return NULL;
...
unsigned char *load_measurement(char *conf)
{
	...
	measurement = kmalloc(32, GFP_KERNEL);
...
sigstruct_t *load_sigstruct(char *conf)
{
	...
	printk(KERN_INFO "# debug sigstruct:\n%s\n", msg);
	kfree(msg);
#endif
...
einittoken_t *load_einittoken(char *conf)
{
	...
	printk(KERN_INFO "# debug einittoken:\n%s\n", msg);
	kfree(msg);
#endif
...
void hexdump(char **fd, void *addr, int len)
{
	int i;
