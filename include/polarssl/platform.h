...
#define polarssl_malloc(v)     kmalloc(v, GFP_KERNEL)
#define polarssl_free       kfree
...
#define polarssl_printf(...)     printk(KERN_INFO __VA_ARGS__)
...
int platform_set_fprintf( int (*fprintf_func)( FILE *stream, const char *,
                                               ... ) );
#else
#define polarssl_fprintf(f, ...)    printk(KERN_INFO __VA_ARGS__)
#endif
